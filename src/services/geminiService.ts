...
export async function rephraseToAcademic(text: string): Promise<string> {
  ...
  try {
    ...
    return "Error: Could not connect to the AI assistant. Please try again later.";
  }
}
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        ...
        `أنت خبير محترف في تصميم العروض التقديمية الأكاديمية (Presentation Designer).
    ...
    return "حدث خطأ أثناء محاولة تحليل الشريحة. يرجى المحاولة مرة أخرى.";
  }
}
